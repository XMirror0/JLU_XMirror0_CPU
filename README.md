...
在用Logisim做了一个简单的9指令MIPS流水线CPU后，再用Verilog做一个46指令的LA32R流水线CPU,以前很多不明白，不清晰的点都明晰了很多。Verilog中有三个概念叫做：**门级描述**、**数据流级描述**和**行为级描述**。我们的课本更多的是**数据流级描述**，而我们面对Logisim则要使用**门级描述**，导致我们知道什么时候数据该怎么流动，但是却不知道电路怎么解读这些数据流，电路该做出什么样的行为。往往是“一看就会，一做就废”